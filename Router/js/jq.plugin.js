...
(function (argument) {
    var Placeholder = function () {
        ...
        if('placeholder' in newInput){
            return;
        }
        var items = $('input[placeholder]');
        items.each(function (i) {
            ...
            if(!cur)
            {
                self.val(holder).addClass('input-holder');
            }
            self.on('focus', function (e) {
                if(!cur){
                    self.val('').removeClass('input-holder');
                }
            }).on('blur', function (e) {
                cur = $.trim(self.val());
                if(!cur){
...
$(document).ajaxError(function(event, jqxhr, settings, thrownError) {
    if(settings.url == '/router/get_user_type.cgi'){
...
(function ($) {
    var _ajax = $.ajax;
    var getQueryString=function(locationEmt){
    ...
    $.ajax = function () {
        ...
        var _complete = obj[0]["dataFilter"] || $.noop;
        obj[0]["timeout"]=15*1000;
        obj[0]["beforeSend"]=function(xhr){
            if((typeof current_html!="undefined")&&current_html=="login"){
                return true;
            }
           var  token_id= getQueryString(window.top)["token_id"];
            xhr.setRequestHeader("token_id",token_id)
        };
        obj[0]["dataFilter"] = function (data, dataType) {
            if (data.indexOf("360LoginFlag") != -1) {
...
//input watch
(function($) {  
    $.fn.isChange = function(callback) {  
        return this.each(function() {  
            //缓存以前的值  
            $.data(this, 'originVal', $(this).val());  
  
            //event  
            $(this).unbind("keyup paste").bind('keyup paste', function() {  
